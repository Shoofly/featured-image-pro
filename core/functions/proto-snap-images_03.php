...
if ( !class_exists( 'featured_image_pro_image_retrieve_03' ) ):
	...
	function wordpress_featured_images( $attobject, $atts, $options ) {
		...
		$proto_post->subcaption[$subcaption1]  = '';
		$proto_post->subcaption[$subcaption2]  = '';
		$proto_post->subcaption[$subcaption1] = apply_filters('proto_subcaption', $proto_post->subcaption[$subcaption1], $subcaption1, $postid);
		$proto_post->subcaption[$subcaption2] = apply_filters('proto_subcaption', $proto_post->subcaption[$subcaption2], $subcaption2, $postid);
		...
		$proto_post->link_url = get_permalink($postid);//, true);  //Link to post

		$proto_post->excerpt = $this->proto_get_post_excerpt($qpost, $options);
	 ...
	 * @return excerpt
	 */
	function proto_get_post_excerpt( $qpost, $options )
	{
		$excerpt = '';
		if ( isset( $options['showexcerpts'] ) && $options['showexcerpts'] ) {

			$excerpt_length_default = apply_filters( 'excerpt_length', 55 );
			if ( isset( $options['excerptlength'] ) ) {
				$excerpt_length = intval( $options['excerptlength'] );
				if ( $excerpt_length == 0 )
					$excerpt_length = $excerpt_length_default;
			}
			else
				$excerpt_length = $excerpt_length_default;

			if (isset ($qpost->post_excerpt) && $qpost->post_excerpt != '')
				$excerpt = $this->proto_trim_excerpt( $qpost->post_excerpt, $excerpt_length ); //need to do this ourselves so that shortcode is not executed as in the wordpress
			else
				$excerpt = $this->proto_trim_excerpt( $qpost->post_content, $excerpt_length ); //need to do this ourselves so that shortcode is not executed as in the wordpress code which may call the_content at the time of development. This is just wrong.
			$this->read_more = '';;
	 ...
	 */
	function proto_trim_excerpt( $text, $excerptlength = 55, $read_more = '' ) {
	...
	function custom_link_button( $post_id, $options ) {
		...
		$type = isset( $options['excerpt_custom_link_style'] ) ? sanitize_text_field( $options['excerpt_custom_link_style'] ) : 'button';
